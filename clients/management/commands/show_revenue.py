...
class Command(BaseCommand):
    help = 'Display all existing client revenue data from database'
    ...
    def handle(self, *args, **kwargs):
        ...
        for client in clients:
            print(f"{client.id:<3} {client.company_name[:34]:<35} ₹{str(client.total_pitched_amount):<10} {str(client.gst_percentage):<5}% ₹{str(client.gst_amount):<10} ₹{str(client.total_with_gst):<10} ₹{str(client.received_amount):<10} ₹{str(client.pending_amount):<10}")
        ...
        print("-"*110)
        print(f"{'TOTAL':<3} {'':<35} ₹{str(total_pitched):<10} {'AVG':<5}  ₹{str(total_gst):<10} ₹{str(total_with_gst):<10} ₹{str(total_received):<10} ₹{str(total_pending):<10}")
        print("="*110)
